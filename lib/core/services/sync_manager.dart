...
class SyncManager {
  ...
  /// Inisialisasi dan mulai mendengarkan perubahan koneksi.
  ...
  void startListening() {
    ...
    // Coba sync langsung saat pertama kali app buka
    _trySyncOnStart();
  }
  ...
  /// Alias lama untuk syncAll (untuk kompatibilitas dengan file lain)
  Future<void> syncPendingData() async {
    await syncAll();
  }

  /// Proses semua item di antrian (Sync All).
  ...
  Future<int> syncAll() async {
    ...
    try {
      // 1. Jalankan sinkronisasi berbasis Queue (FIFO)
      ...
      // 2. Fallback: Sync manual untuk data yang tersisa di Box utama tapi belum isSynced
  ...
  Future<void> enqueueBookmark({
    ...
    // Menentukan action name dengan kompatibilitas lama
    final actionName = action ?? (isAdd ? SyncAction.bookmarkAdd.name : SyncAction.bookmarkRemove.name);
  ...
  /// Alias lawas untuk enqueueBookmark
  Future<void> enqueueBookmarkAction({
    required String userId,
    required String questionId,
    required String action,
  }) async {
    final isAdd = action == SyncAction.bookmarkAdd.name || action == 'add';
    await enqueueBookmark(
      userId: userId, 
  ...
  // ─── Private: Sync Logika Khusus ─────────────────────────────────────────

  Future<void> _syncBookmarkQueue(SyncQueueModel item) async {
    final payload = item.payload;
    // Mendukung dua versi key payload (versi camelCase dan snake_case)
    final userId = payload['userId']?.toString() ?? payload['user_id']?.toString() ?? '';
    final questionId = payload['questionId']?.toString() ?? payload['question_id']?.toString() ?? '';
    
    if (userId.isEmpty || questionId.isEmpty) return;

    if (item.action == SyncAction.bookmarkAdd.name || payload['action'] == 'add') {
      await _bookmarkRemote.addBookmark(userId: userId, questionId: questionId);
    } else if (item.action == SyncAction.bookmarkRemove.name || payload['action'] == 'remove') {
      await _bookmarkRemote.removeBookmark(userId: userId, questionId: questionId);
    }
  }
